...
internal sealed class GetAllOrganizers : IEndpoint
{
   public void MapEndpoint(IEndpointRouteBuilder app)
   {
      app.MapGet("organizers", async (ISender sender) =>
         {
            Result<IReadOnlyCollection<OrganizerDto>> result = await sender.Send(new GetAllOrganizersQuery());

            return result.Match(Results.Ok, ApiResults.Problem);
         })
      .WithTags(Tags.Organizers)
      .Produces<Result<IReadOnlyCollection<OrganizerDto>>>()
